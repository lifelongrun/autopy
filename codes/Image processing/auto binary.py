...
def process_flame_image(image_path):
    ...
    ksize = int(min(height, width) * 0.08) # 根据图像尺寸计算锐化核的大小
    if ksize % 2 == 0:
        ksize += 1
    print(f"Ksize: {ksize}")
    # 定义一个锐化的核
...
def process_folder_and_get_statistics(folder_path, image_height_mm=500):
    ...
    for jpg_file in jpg_files:
        binary_image = process_flame_image(jpg_file) # 调用函数:处理图像
        index = os.path.basename(jpg_file)
        if binary_image is not None:
            flame_height_mm, _ = find_fire_height(binary_image, image_height_mm) # 调用函数:计算火焰高度
...
# 示例用法
folder_path = r'E:\Github-autopy\codes\Image processing'  # 替换为实际文件夹路径
mean_height, std_deviation = process_folder_and_get_statistics(folder_path)
print("均值:", mean_height)
